...
This means that tipically in a template we will have:
...
<3> Specification for the DeploymentConfig. Everything inside this section describes the DeploymentConfig's configuration.
...
As this example, uses 2 pods, one as the frontend application (web), and another as a database, we need to link both containers, so that we do not need to do any manual extra configuration after the deployment is made for the fronend to be able to access the database. We need to inject into the frontend DeploymentConfig values relative from the database DeploymentConfig.

The database DeploymentConfig expects 3 ENV variables to set up the username, password and database as https://docs.openshift.com/enterprise/3.0/using_images/db_images/mysql.html#environment-variables[documented here]. (There are other variables for configuraing the MySQL database behavior)
...
Now we need to configure when our application will be deployed/redeployed. We are going to modify the DeploymentConfigs that we have created to set some additional behavior.
...
Now we have everything that is needed for our application to be running, but we might want to access parts any of the components of our application either internally (for troubleshooting, inter applications communications) using for it https://docs.openshift.com/enterprise/3.0/architecture/core_concepts/pods_and_services.html#services[Services] or externally, defining for it, https://docs.openshift.com/enterprise/3.0/dev_guide/routes.html[Routes].
...
The following service defines access to the web component using HTTP tranport on port 8080:
...
This service defines access to the web component using HTTPS tranport on port 8443:
...
=== Create routes
Now that our web component has a service, we can create a Route for those services, and provide with an exteranl way of accessing our web component in a DNS name registered for our platform.
...
----
<1> Name of the Route
<2> Labels decarating this Route
...
As we want to identify univoquely the resources we are deploying as a whole, it is important that all of them have at least one label for this purpose. In the previous code we have set in all of the resources a label of:
...
A property placeholder will loke like:
...
* Parameters with autogenerated values (using a regexp like expression)
...
We will execute the creation of the template as a user in the current project. (The user will need to have the appropriate rols to create "Template" resources in the current project)
...
If the user belongs to multiple projetcs, and wants to create the template in a different project from the one he's currently working on, he can do it with *-n <project>*.
...
For viewing all the available templates for use (using the CLI) we will have to, list the templates in the "openshift project" and in the users current project.
...
== Caveats - Things you should remember
Lastly, I will provide you with a list of things you need to remember.


* Templates can be shared or per-project